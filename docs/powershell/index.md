...
* An authentication option. One of:
	...
	* Certificate (uses the local Orion server certificate. This will only work when running `SwisPowerShell` on an Orion server)
...
Installations utilizing the legacy SolarWinds Virtualization Manager appliance also support SWIS. You can connect to the appliance using `Connect-Swis`, but the options needed are different. Use this command line:
