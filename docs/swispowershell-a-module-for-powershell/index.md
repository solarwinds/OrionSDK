...
1. Visit https://github.com/solarwinds/OrionSDK, click the green "Clone or download" button, and then click "Download ZIP".

2. Extract the zip to a temp directory.

3. Copy the `SwisPowerShell` directory from the `Samples\PowerShell` directory you just extract to either:

   1. `$pshome\Modules` (`%windir%\System32\WindowsPowerShell\v1.0\Modules`) - this will install SwisPowerShell system-wide.

   2. `$home\Documents\WindowsPowerShell\Modules` (`%UserProfile%\Documents\WindowsPowerShell\Modules`) - this will install PowerShell for the current user only.
