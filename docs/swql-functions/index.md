...
* `GetUtcDate()` - Returns the current date and time in UTC. Please note [possible issues](possible-issues/#datetime-functions).
...
* `SplitStringToArray(a)` - Splits string `a` on comma separators into an array of substrings.
