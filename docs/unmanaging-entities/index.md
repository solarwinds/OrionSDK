...
To unmanage a node, use the `Orion.Nodes.Unmanage` verb. Its signature looks like this:
...
* `netObjectId` - the identity of the node to unmanage. It looks like `N:123` where 123 is the NodeID.
* `unmanageFrom` - the date and time (in UTC) to begin the unmanage period. If this is in the past, the node will be unmanaged effective immediately.
* `unmanageUntil` - the date and time (in UTC) to end the unmanage period. You can set this as far in the future as you like.
* `isRelative` - if `true`, then the `unmanageUntil` argument will be interpreted differently: the date portion will be ignored and the time portion will be treated as a *duration*. I recommend passing `false` for `isRelative` - it makes the scripts more clear and consistent.
...
To end the unmanage period for a node early (before the scheduled `unmanageUntil` time), you can call the `Orion.Nodes.Remanage` verb. Its signature looks like this:
